...
util.fileDownload = function(url, $from) {
	...
	var loadObj = $.Prompt('<div id="bestMeet_loading" class="loading spin"></div>', 0, {
		bg : true
	}); 
...
util.bestMeetAjax = function(param) {
	var params = {
		type : 'post',
	...
	obj.complete = function(request, textStatus) {
		...
		if (params.tokenKey) {
			delete bestMeetToken[params.tokenKey];
		}
	...
	if (params.loading) {
		loadObj = $.Prompt('<div id="bestMeet_loading" class="loading spin"></div>', 0, {
			bg : true
		}); 
	...
	if (!bestMeetToken[params.tokenKey]) {
		return $.ajax(obj);
	}
};
